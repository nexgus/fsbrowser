...
在 `"dir"` 模式下, 未選取任何項目即確認時, 會回傳目前正在瀏覽的目錄, 與系統本身的選擇資料夾對話框行為一致; 這要求目錄已經載入完成, 因此面板完成初次列出之前無法確認.
...
- 在列表空白處單純點擊 (未按 Ctrl/Cmd/Shift) 會清除全部選取, 與 Finder / Windows 檔案總管一致; 重新命名或新增資料夾進行中不清除.
...
`returnMode="multiple"` 只要求至少選取一列, 沒有上限 -- 但 `"dir"` 模式例外, 未選取任何項目時仍可確認 (見 2.2 節).
