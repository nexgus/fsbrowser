#!/usr/bin/env bash
# 建置 examples/vue3 這個 Wails v3 範例 app 至 examples/vue3/bin/ (該目錄不入 git).
#
...
GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build \
  ...
  -o bin/fsbrowser-example-vue3-windows-amd64.exe \
  .

echo "==> 建置 macOS $(go env GOARCH) (cgo)"
CGO_ENABLED=1 go build \
  -o "bin/fsbrowser-example-vue3-darwin-$(go env GOARCH)" \
  .
